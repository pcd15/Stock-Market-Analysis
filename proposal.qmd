...
With data set 1, we would like to answer the following question : How have interest rates and inflation rates historically impacted the stock market? Tracking historical trends can be useful in maximizing profit from the bullish or bearish market, making it significant for investors or fintech workers. Our data assess the impact of quantitative variables such as interest rate and inflation rate. It also assesses categorical variables such as time and whether or not interest rate and inflation rate rose or fell (TRUE if they rose, FALSE if they fell). Our hypothesis is that the stock market, measured by the S&P 500 index, would be hurt by high inflation and corresponding high inflation rates, because consumer spending drops.
...
historicaldata <- inner_join(historicaldata, sp500_year, by = "year")
...
The source of the data is DATA HUB. This data was collected from S&P Dow Jones Indices which is collected daily. The observation shows that over the course of the year the share price has gone down, with the exception of October where it went up 2.24% and then down again. The year to date return was -23.09%, and month to date return was 2.24%.

## Research question

Today, the Information Technology sector is leading the way. Gathering data on how different businesses are performing, and whether this correlates with the industry they are in is useful to indicate whether performance patterns are caused by individual businesses or specific sectors and strategies can be derived from each. Our hypothesis is the following: Non-essential industries will go down more than essential industries. The type of variables in our research question are mainly quantitative, analyzing inflation rate, interest rate and comparing these across sectors.
