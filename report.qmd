...
```{r}
#| l label: gains-and-losses-per-month
sp500_curr|>
  mutate(Date = paste(substr(Date, 9, 10), substr(Date, 1, 2),  sep = "/"))|>
  group_by(Date)|>
  count(gain) |>
  ggplot(aes(x = Date, y = n, color = gain)) +
  geom_point() +
