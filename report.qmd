...
Contextualization: Because of high inflation rates, and rising interest rates, the stock market has taken a major blow this past year. However, some sectors and stocks have been more negatively affected than others. Industries that are considered more essential tend to be less affected by fluctuations in these rates. So how true is this, and how much are both of these industries affected?
...
From this graph, it can be seen that over 80% of the stocks in the S&P 500 actually fell. But can we dig deeper? Is there something that these stocks have in common?
...
From these two, we can see that a lot of the stocks that have similar performance also share a sector. So which sectors are doing the best? We will view their performance by calculating the mean return of all the stocks in that given sector, and sorting from best to worst. After that, we will create a graph that shows a similar picture to the first graph in this section. It will detail how many stocks increased and decreased, but it will be faceted by sector, so we can dig deeper into this picture.
...
These results help paint a bigger picture. We can see that energy, utilities, and consumer staples are definitely doing the best from both depictions, and IT and real estate are among the few at the bottom. What if we want to delve deeper into each sector, and examine the maximums and minimums, and what the values hover around?
