...
library(lubridate)
library(dplyr)
>>>>>>> 557825c5e82095891135b1d866301c7cf2d69dce
```
...
We first graphed each day's closing price to determine the trend in closing price over time. We utilized a scatterplot graph to visualize the data for closing price from 10/01/2020 to 10/28/2022. In the graph above, we can see that the closing prices follow a generally positive trend from October 2022 to December 2021 and then a negative trend from January 2022 to October 2022. Each month had a wide range of closing prices that we can observe from the range of the heights of closing price data points each month. Some months have a range of approximately \$250 (i.e. August 2022) and some have a range of approximately \$750 (i.e. February 2021).
...
sp500_curr|>
  ...
  labs(title = "Proportion of Days with Gains and Losses by Month", 
       y = "Proportion of days in month", x = "Month (year/month)") +
  theme_minimal() +
  scale_color_viridis_d()

```

Then we plotted the proportion of daily gains and losses by each month in the SP500. To do this, we first grouped by dates and added a count of the total days in each month recorded. Then we plotted with ggplot and geom_col(), making our x-axis the date/month, our y-axis the proportion of days that gained and lost from its open price to closing price, and filled the bars with said proportion of days that gained and lost from open to closing price. We notice that 7 of the 26 months in our dataset have losses or false gains according to our plot. This means that the other 19 months either had even or higher proportions of days whose closing price was higher than the opening price.
...
sp500_curr|>
  ...
  group_by(new_date)|>
  summarise(mean_dailydiff = mean(daily_diff), mean_price = mean(Price), .groups = "drop") |>
  ggplot(aes(x = new_date, y = mean_dailydiff)) +
  geom_smooth(aes(y = mean_dailydiff), se = FALSE,  colour = "red") +
  labs(
    x = "Date",
...
sp500_curr|>
  ...
  summarise(mean_dailydiff = mean(daily_diff), mean_price = mean(Price), .groups = "drop") |>
  ggplot(aes(x = new_date, y = mean_dailydiff)) +
  geom_smooth(aes(y = mean_price), se= FALSE, colour = "blue") +
  labs(
    x = "Date",
